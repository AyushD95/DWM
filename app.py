...
def product_selection():
    ...
    data = load_data()
    products = data[(data['Category'] == selected_category) & (data['Sub-Category'] == selected_subcategory)]['Product Name'].unique()
    return render_template('product_selection.html', category=selected_category, subcategory=selected_subcategory, products=products)
...
def product_detail(product_name):
    ...
    # Convert sales plot to PNG image
    img = io.BytesIO()
    ...
    if len(product_sales_ts) >= 24:  # Check for sufficient data for forecasting
        ...
        forecast_message = "Insufficient data for forecasting. A minimum of 24 months of sales data is required."
    ...
    return render_template('product_detail.html',
                           ...
                           forecast_message=forecast_message)  # Pass the message to the template
